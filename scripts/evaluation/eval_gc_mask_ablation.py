#!/usr/bin/env python3
"""Evaluate GC closed-loop performance across mask-mode-trained SSA checkpoints."""
    ...
    *,
    ...
    with torch.no_grad():
        for step in range(int(max_steps)):
            ...
            current_block += 1
            state_tokens = [token_mapper.STATE]
    ...
    *,
    ...
    for idx, adjacency in enumerate(instances):
        ...
        if (idx + 1) % 25 == 0:
            ...
            logger.info(
                "mask_mode=%s processed=%d/%d solve_rate=%.3f repeat_rate=%.3f mean_backtracks=%.2f",
                str(mask_mode),
                int(idx + 1),
                int(len(instances)),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate GC mask-mode ablations from SSA checkpoints"
    )
    ...
    for idx, ckpt_raw in enumerate(checkpoint_paths):
        ...
        logger.info(
            "completed label=%s solve_rate=%.3f repeat_rate=%.3f mean_backtracks=%.2f val_loss=%s val_acc=%s",
            label,
