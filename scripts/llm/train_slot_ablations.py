#!/usr/bin/env python3
"""Train SlotLMWrapper verification head with slot persistence ablations.

Variants:
- normal: baseline slot behavior
- no_write: slots update, tokens stay unchanged
- reset_per_layer: slots reset to initial embedding at each layer
- shuffled_slots: slot identities permuted between layers
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="normal",
        choices=["normal", "no_write", "reset_per_layer", "shuffled_slots"],
        help="Slot persistence ablation variant",
    )
    ...
    logger.info(
        "Loading model %s with %d slots (dim=%d), variant=%s...",
        args.model,
        args.n_slots,
        args.slot_dim,
        args.variant,
