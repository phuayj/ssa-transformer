"""Reusable SAT history-transplant trace utilities.

This module exposes the deterministic rollout and canonical-state helpers used
by the manuscript diagnostics without making downstream scripts import from the
...
def _load_reference() -> ModuleType:
    ...
    path = root / "scripts" / "evaluation" / "eval_sat_history_transplant.py"
    spec = importlib.util.spec_from_file_location("_sat_history_transplant_ref", path)
    if spec is None or spec.loader is None:
        raise ImportError(f"could not load SAT transplant reference from {path}")
