#!/usr/bin/env python3
"""Train SSASlotDecoder on Blocks World traces augmented with TRIED markers."""
...
def compute_block_ids(sequence: Sequence[int]) -> List[int]:
    """Compute SSA block IDs from a BW token sequence.

    block_id=0 for fixed prefix (before first STATE/TRIED), then 1..K for decision
    blocks. TRIED starts a block, and STATE immediately following TRIED's END_TRIED
    stays in that same block (TRIED→STATE grouping).
    """
    state_tok = int(BlocksWorldTokenizer.STATE)
    tried_tok = int(BlocksWorldTokenizer.TRIED)
...
def _collate(batch):
    ...
    input_ids = torch.full(
        (bsz, max_len), int(BlocksWorldTokenizer.PAD), dtype=torch.long
    )
...
def _compute_lm_loss(
    ...
    labels = labels.masked_fill(~supervised, int(BlocksWorldTokenizer.PAD))
    loss = F.cross_entropy(
        shifted_logits.reshape(-1, shifted_logits.size(-1)),
        labels.reshape(-1),
        ignore_index=int(BlocksWorldTokenizer.PAD),
    )
        ...
        if not logged:
            ...
            if str(mode) == "ssa":
                ...
                if valid_block_ids.numel() > 0:
                    logger.info(
                        "sample_blocks train=%s mode=%s goal_prefix_tokens=%d n_blocks=%d",
                        str(train),
                        str(mode),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train BW SSA Slot decoder with next-token loss"
    )
    ...
    tokenizer = BlocksWorldTokenizer()
    vocab_size = int(BlocksWorldTokenizer.VOCAB_SIZE)
