#!/usr/bin/env python3
"""Generate graph-coloring traces with explicit TRIED markers after backtracks."""
    ...
    *,
    ...
    for _step in range(int(max_steps)):
        ...
        if _is_solution(env, state):
            _append(tokens, [tokenizer.SOLVED], int(max_seq_len))
            break
        if state.status != GraphColorEnvStatus.RUNNING:
            _append(tokens, [tokenizer.FAILED], int(max_seq_len))
            break
        ...
        if prior:
            ...
            if not _append(tokens, tried_tokens, int(max_seq_len)):
                return {
                    "ok": False,
                    "reason": "max_seq_len",
                    "sequence": tokens,
        ...
        if min_domain_size == 0:
            if not _append(tokens, [tokenizer.CF], int(max_seq_len)):
                return {
                    "ok": False,
                    "reason": "max_seq_len",
                    "sequence": tokens,
        ...
        if not _append(tokens, state_tokens, int(max_seq_len)):
            return {
                "ok": False,
                "reason": "max_seq_len",
                "sequence": tokens,
        ...
        if not _append(tokens, [tokenizer.OK], int(max_seq_len)):
            return {
                "ok": False,
                "reason": "max_seq_len",
                "sequence": tokens,
        ...
        if selected_node is None:
            _append(tokens, [tokenizer.CF], int(max_seq_len))
            if not state.assignment_stack:
                _append(tokens, [tokenizer.FAILED], int(max_seq_len))
                break
            env.backjump_to(len(state.assignment_stack) - 1)
        ...
        if not _append(tokens, [tokenizer.node_token(selected_node)], int(max_seq_len)):
            return {
                "ok": False,
                "reason": "max_seq_len",
                "sequence": tokens,
        ...
        oracle_color = int(min(domain))
        alternatives = sorted(int(c) for c in domain if int(c) != int(oracle_color))
        ...
        if not _append(tokens, [tokenizer.mask_token(domain)], int(max_seq_len)):
            return {
                "ok": False,
                "reason": "max_seq_len",
                "sequence": tokens,
        ...
        if not _append(tokens, [tokenizer.color_token(chosen_color)], int(max_seq_len)):
            return {
                "ok": False,
                "reason": "max_seq_len",
                "sequence": tokens,
    ...
    if not tokens or tokens[-1] != int(tokenizer.EOS):
        _append(tokens, [tokenizer.EOS], int(max_seq_len))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate GC traces with explicit TRIED branch-history markers"
    )
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="experiments/trajectories/gc_tried_traces_n30.pkl",
    )
    ...
    for idx in range(int(args.num_instances)):
        ...
        examples.append({"sequence": seq, "loss_mask": lm})
