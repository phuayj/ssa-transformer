#!/usr/bin/env python3
"""Train GC SSA models under different attention mask ablations."""
...
def _get_state_tried_tokens(vocab_size: int) -> Tuple[int, int]:
    if int(vocab_size) <= 120:
        # Parsing domain: STATE=4, no TRIED token
        return 4, -1
    elif int(vocab_size) <= 394:
        # n=30 layout
        ...
        # SAT interleaved tokenizer: STATE=6, no TRIED token
        return 6, -1
    else:
...
def compute_block_ids_for_vocab(sequence: Sequence[int], vocab_size: int) -> List[int]:
    """Compute SSA block IDs from a token sequence for a given vocab layout.
...
class SSATriedDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[List[int], List[bool], List[int]]:
        ...
        lm = [bool(x) for x in item["loss_mask"]][: self.max_seq_len]
        if len(seq) != len(lm):
            raise ValueError("sequence/loss_mask mismatch")

        # Use pre-computed block_ids if available, otherwise compute from tokens.
        if "block_ids" in item:
            blk = [int(x) for x in item["block_ids"]][: self.max_seq_len]
        else:
            blk = compute_block_ids_for_vocab(seq, vocab_size=self.vocab_size)
        if len(blk) != len(seq):
...
def _run_epoch(
    *,
    ...
    for input_ids, attention_mask, loss_mask, block_ids in loader:
        ...
        if not logged:
            ...
            if valid_pos.numel() > 0:
                ...
                logger.info(
                    "sample_token train=%s mask_mode=%s pos=%d target=%s pred=%s",
                    str(train),
                    str(mask_mode),
                    int(p),
            ...
            if valid_block_ids.numel() > 0:
                logger.info(
                    "sample_blocks train=%s mask_mode=%s graph_prefix_tokens=%d n_blocks=%d",
                    str(train),
                    str(mask_mode),
    ...
    stats = {
        ...
        "mean_blocks_per_seq": float(total_blocks / max(float(total_seq), 1.0)),
    }
    return stats
...
def _infer_vocab_size_from_records(records: Sequence[Dict[str, Any]]) -> int:
    ...
    logger.info("inferred vocab_size=%d from training data", int(vocab_size))
    return vocab_size
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train GC SSA decoder with mask ablation"
    )
    ...
    parser.add_argument(
        "--smoke",
        action="store_true",
        help="Run one train/val batch and write smoke.json without training.",
    )
    ...
    vocab_size = _infer_vocab_size_from_records(records)
    _get_state_tried_tokens(vocab_size)
    ...
    if bool(args.smoke):
        ...
        with torch.no_grad():
            ...
            train_loss, train_token_count, train_supervised, train_labels = _compute_lm_loss(
                logits=train_logits,
            ...
            train_correct = int(((train_preds == train_labels) & train_supervised).sum().item())
    ...
    logger.info(
        "start_training model_type=%s block_mode=%s mask_mode=%s train=%d val=%d vocab_size=%d batch=%d epochs=%d",
    ...
    for epoch in range(int(args.epochs)):
        ...
        logger.info(
            "epoch=%d/%d mask_mode=%s train_loss=%.4f train_acc=%.4f val_loss=%.4f val_acc=%.4f lr=%.2e",
            int(epoch + 1),
            int(args.epochs),
            str(args.mask_mode),
    ...
    logger.info(
        "training_complete mask_mode=%s best_val_loss=%.4f output_dir=%s",
        str(args.mask_mode),
        best_val,
        output_dir,
    )
