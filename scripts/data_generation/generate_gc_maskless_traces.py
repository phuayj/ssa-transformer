#!/usr/bin/env python3
"""Generate graph-coloring traces with structural domains, TRIED markers, and no mask token."""
...
def _build_single_trace(
    *,
    ...
    for _step in range(int(max_steps)):
        ...
        # Domain is still used for oracle selection but is intentionally NOT serialized.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate GC traces with structural domains and TRIED markers (maskless)"
    )
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="experiments/gc-maskless-traces/traces.pkl",
    )
